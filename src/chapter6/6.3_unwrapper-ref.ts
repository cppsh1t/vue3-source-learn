...
type RefWrapper<T> = { value: T }
type ToRefsWrapper<T> = {
    [K in keyof T]: { value: T[keyof T] }
}
type UnWrapperRef<U> = U extends ToRefsWrapper<infer T> ? {
...
function ref<T>(initValue: T): RefWrapper<T> {
    const wrapper = {
        value: initValue,
    }
...
function proxyRefs<T, U extends ToRefsWrapper<T>>(target: U): UnWrapperRef<U> {
    ...
    }) as unknown as UnWrapperRef<U>
}

export { effect, computed, watch, reactive, shallowReactive, readonly, shallowReadonly, ref, toRef, toRefs, proxyRefs }
