...
import { createRenderer } from "./10.1_double-diff"
...
test('move-element-test', async () => {
    ...
    const newVNode: VNode = {
        ...
        children: [
            ...
            { type: 'p', children: '1 now 2', key: 1, props: { id: 'ele1' } },
            { type: 'p', children: '2 now 3', key: 2, props: { id: 'ele2' } },
        ],
    }
    ...
    expect(document.querySelector('#foo')?.children[2].id).toEqual('ele2')
})
...
test('remove-element-test', async () => {
    ...
    expect(document.querySelector('#foo')?.children[1].id).toEqual('ele1')
    expect(document.querySelector('#foo')?.children[2].id).toEqual('ele2')
    expect(document.querySelector('#foo')?.children).toHaveLength(3)
})
